...
class ScaledMultiHeadAttention(nn.Module):
    ...
    def forward(self, query, key, value, mask=None):
        ...
        relative_positions_embeddings = self.relative_positional_encoding(length).to(query.device)
        ...
        value_out = value_out.transpose(1, 2).contiguous().view(batch_size, -1, self.n_dim)
...
class RelativePositionalEncoding(nn.Module):
    ...
    def forward(self, length):
        ...
        relative_positions_embeddings = self.relative_positions_matrix.index_select(0, positions.view(-1)).view(length, length, -1)
